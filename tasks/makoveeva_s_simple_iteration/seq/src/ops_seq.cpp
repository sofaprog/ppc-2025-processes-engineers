...
void Iterate(int n, const std::vector<double> &a, const std::vector<double> &b, std::vector<double> *x) {
  ...
  for (int iter = 0; iter < kMaxIter; ++iter) {
    ...
    for (int i = 0; i < n; ++i) {
      ...
      double sum = 0.0;
      for (int j = 0; j < n; ++j) {
        sum += a[i_idx * n_sz + static_cast<size_t>(j)] * (*x)[static_cast<size_t>(j)];
      }

      const double xi = (*x)[i_idx] + (kW * (b[i_idx] - sum) / a[i_idx * n_sz + i_idx]);
      x_new[i_idx] = xi;
...
bool MakoveevaSSimpleIterationSEQ::PreProcessingImpl() {
  GetOutput() = 0;  // важно для повторных прогонов в perf/pipeline
  return true;
}
...
bool MakoveevaSSimpleIterationSEQ::PostProcessingImpl() {
  return true;  // корректность проверяется тестами (CheckTestOutputData)
}
